...
| `NEXT_PUBLIC_API_URL` | Base URL for the Comm-Fit API, exposed to the browser. Local: `http://localhost:3000`. | `https://api-production-9f18c.up.railway.app` | No | Yes | No trailing slash. Used by `@commfit/sdk`. **Railway assigns random subdomains — verify the current URL in Railway dashboard after every new API service creation.** The placeholder `https://commfit-api.up.railway.app` set during M2 was incorrect and required a redeploy of all 3 Vercel apps to fix (COM-54). |
