...
>(({ className, children, label, error, ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      {label && (
        <label htmlFor={triggerId} className="text-xs font-medium text-text-primary">
          {label}
        </label>
      )}
      <SelectPrimitive.Trigger
        ...
        id={triggerId}
        className={cn(
          "flex h-9 w-full items-center justify-between rounded border border-border bg-surface px-3 text-sm text-text-primary",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded py-1.5 pl-8 pr-2 text-sm text-text-primary",
